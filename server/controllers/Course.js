...
exports.editCourse = async (req, res) => {
  try {
    ...
    // Instructor-approval gate: block publishing until the instructor is approved
    if (updates.status === "Published") {
      const instructor = await User.findById(course.instructor).select(
        "approved accountType",
      );
      if (
        instructor &&
        ...
        return res.status(403).json({
          success: false,
          message:
            "Your instructor account is pending admin approval. You can build courses but cannot publish yet.",
        });
      }
    }
